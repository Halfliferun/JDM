...
banner.prototype.initSwipe = function() {
  ...
  that.imageBox.addEventListener("touchstart", function(e) {
    ...
    time = Date.now();
    clearInterval(that.timer); //滑动的时候，禁用自动轮播功能
  });
  that.imageBox.addEventListener("touchmove", function(e) {
    ...
    // 计算图片容器应该做的位移是多少
    // 将要去做定位的位置 = 现在的位置 + 滑动的距离（不分正负）(因为位置是基于原来的位置去滑动的)
    var translateX = -that.index * that.width + distance;
    that.removeTransition();
    // 注意这里不能有动画
    that.setTranslateX(translateX);
  });
  that.imageBox.addEventListener("touchend", function(e) {
    // console.log("ee");
    // 速度=路程/时间
    // 路程：离开屏幕的点和开始触摸屏的点的距离
    var s = Math.abs(distance);
    // 时间：离开屏幕的时间和开始触摸屏幕的时间的差
    var t = Date.now() - time;
    var v = s / t;
    // console.log(v); //px/ms 手速在0.25以上为快
    // 注意：速度要优先于滑动的距离，因此判断要放到上面
    if (v > 0.25) {
      if (distance > 0) {
        // 右滑动 上一张
        that.index--;
      ...
      // // 1:滑动的距离 取绝对值 去判断是否超过三分之一,完成吸附功能
      if (Math.abs(distance) < that.swipeWidth) {
        var translateX = -that.index * that.width;
        that.addTransition();
        that.setTranslateX(translateX);
      } else {
...
// ===============================================================
//备忘录：
// 今天因为没有清除浮动，造成touchmove一直触发不了，牢记血的教训！！整整浪费了一个下午。
// 论一个好习惯的重要性和浮动是魔鬼的真实案例
// ==============================================================
